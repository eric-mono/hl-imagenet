...
def write_markdown(records: list[LogRecord], path: Path) -> None:
    ...
    with path.open("w") as f:
        ...
        f.write(
            "`logs/phase1/` was historically not a clean Phase 1 directory. Older "
            "evaluator routing sent every tag not starting with `phase2` into "
            "`logs/phase1`, so many later Phase 2 train-only, verify-wave, loop, "
            "and repro runs had to be moved into `logs/phase2`. Use lineage/"
            "classification metadata, not directory names alone, when building "
            "plots.\n\n"
        )
        f.write("## Summary\n\n")
